...
function getIndentationValue<T extends [number, number]>(
  param: { [key: string]: T } | T | undefined,
  breakpoint: string
) {
...
type Props = RGLProps & {
  ...
  // Callbacks
  onBreakpointChange: (b: string, cols: number) => void;
...
> {
  ...
  static propTypes = {
    ...
    layouts(props: Props, propName: string) {
      const obj = props[propName as keyof Props] || {};
      if (type(obj) !== "[object Object]") {
        throw new Error(
          "Layout property must be an object. Received: " + type(obj)
        );
      }
  ...
  generateInitialState(): State {
    ...
    const compactType =
      this.props.verticalCompact === false ? undefined : this.props.compactType;
